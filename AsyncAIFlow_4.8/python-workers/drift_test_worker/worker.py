"""
drift_test_worker — stub worker for drift_test action type.

Behavior: immediately returns SUCCEEDED without running any actual tests.
This keeps the DAG flowing for demo/hackathon purposes.

Environment variables
---------------------
  ASYNCAIFLOW_URL            = http://localhost:8080
  DRIFT_TEST_WORKER_ID       = drift-test-worker-1
  POLL_INTERVAL_S            = 2
"""
...
ACTION_TYPE = "drift_test"

logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [drift-test-worker] %(levelname)s %(message)s",
    datefmt="%Y-%m-%dT%H:%M:%S",
)
LOGGER = logging.getLogger(__name__)

ASYNCAIFLOW_URL: str = os.environ.get("ASYNCAIFLOW_URL", "http://localhost:8080")
WORKER_ID: str = os.environ.get("DRIFT_TEST_WORKER_ID", "drift-test-worker-1")
...
def run_worker() -> None:
    ...
    while True:
        ...
        try:
            ...
            p = {}
        issue = p.get("issue") or "(no issue)"

        LOGGER.info("Stub test pass for issue=%.80s", issue)

        # Stub: all tests pass for demo purposes
        submit_result(action_id, "SUCCEEDED", {
            "test_passed": True,
            "test_count": 0,
            "message": "Tests skipped in demo mode — all checks passed",
            "scope": "drift_backend",
        })
